<?php
    ...
    class MetaFile implements PublisherInterface
    {
         ...
         * @var MetaFile[]
         */
        protected static array $instancesById = [];
         ...
         * @var MetaFile[]
         */
        protected static array$instancesByPath = [];

        /**
         * @var FilesystemFile
         */
        protected FilesystemFile $filesystemFile;
         ...
         * @var MetaFolder
         */
        protected MetaFolder $metaFolder;
        ...
        /**
         * returns a MetaFile instance alternatively identified by its path or its primary key in the database
         *
        ...
        public static function getInstancesByPaths(array $paths): array
        {
            ...
            // build and execute a query, if necessary
        ...
        public static function getMetaFilesInFolder(MetaFolder $folder, ?callable $callBackSort = null): array
        {
            ...
            $files = Application::getInstance()->getVxPDO()->doPreparedQuery("SELECT f.*, CONCAT(fo.path, COALESCE(f.obscured_filename, f.file)) as fullpath FROM files f INNER JOIN folders fo ON f.foldersid = fo.foldersid WHERE fo.foldersid = ?", [$folder->getId()]);
            ...
            if (is_callable("self::$callBackSort")) {
                usort($result, self::class . "::$callBackSort");
                return $result;
            }
        ...
        public static function isFilenameAvailable(string $filename, MetaFolder $f): bool
        {
            ...
            $fullPath = $f->getFullPath() . $filename;

            if (isset(self::$instancesByPath[$fullPath])) {
                return false;
            }
        ...
        private function __construct(?string $path = null, ?int $id = null, ?array $dbEntry = null)
        {
            if (isset($path)) {
                ...
                $this->data = array_change_key_case($dbEntry);
            }
            ...
            // when a record features an obscured_filename, the FilesystemFile is bound to this obscured filename, while the metafile always references the non-obscured filename
        ...
        /**
         * retrieves the physical path of a file
         *
         * @return string
         */
        ...
        /**
         * retrieves the physical filename of the file
         *
         * @return string
         */
        ...
        /**
         * retrieves metafile name of a file
         * differs from the physical filename when a file is obscured
         *
         * @return string
         */
        ...
        public function rename(string $to): void
        {
            ...
            if (!$this->isObscured) {
                ...
                } catch (FilesystemFileException) {
                    throw new MetaFileException(sprintf("Rename from '%s' to '%s' failed. '%s' already exists.", $oldpath, $newpath, $oldpath));
                }
            }

            try {
                Application::getInstance()->getVxPDO()->execute('UPDATE files SET file = ? WHERE filesid = ?', [$to, $this->id]);
            } catch (\Exception) {
        ...
        /**
         * obscures a filename,
         * renames a filesystem file, then updates metafile data in the database, and sets an isObscured flag
         *
        ...
        /**
         * commit changes to metadata by writing data to the database
         */
        ...
        /**
         * creates a meta file based on the filesystem file
         *
         * @param FilesystemFile $file
         * @return MetaFile
         * @throws \Throwable
         */
        public static function createMetaFile(FilesystemFile $file): self
        {
