...
export default {
		template: '<div><ul class="pagination"><li v-if="showNavButtons" class="page-item" :class="{disabled: !hasPrev}"><z-link tabindex="-1" v-on:click="() => prevPage()">{{prevText}}</z-link></li><li v-for="(page, idx) in pagesToShow" v-bind:key="idx" class="page-item" :class="{active: currentPage === page}"><z-link v-on:click="() => pageClick(page)" v-if="page !== &#39;dots&#39;">{{page}}</z-link><span v-else="">...</span></li><li v-if="showNavButtons" class="page-item" :class="{disabled: !hasNext}"><z-link tabindex="-1" v-on:click="() => nextPage()">{{nextText}}</z-link></li></ul></div>',
  name: 'Pagination',
  props,
  data,
  created() {
