    ...
    export default {
		template: '<div class="date-input"><div class="input-group" :style="computedStyles"><div class="form-input" v-if="dateString"><span class="chip">{{ dateString }}<a href="#" class="btn btn-clear" aria-label="Close" role="button" @click.prevent="handleClear"></a></span></div><input v-else="" type="text" class="form-input" autocomplete="off" v-model="inputString" @blur="handleBlur"><button v-if="showButton" type="button" class="btn webfont-icon-only btn-primary input-group-btn" @click.stop="$emit(&#39;toggle-datepicker&#39;)"></button></div></div>',
